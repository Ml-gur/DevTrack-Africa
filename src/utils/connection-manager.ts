...
class ConnectionManager {
  ...
  private handleOffline = () => {
    log.debug('Browser went offline');
    this.status = {
      ...this.status,
      online: false,
      supabaseReachable: false,
  ...
  public async checkConnections(): Promise<void> {
    if (this.isChecking) {
      log.debug('Connection check already in progress');
      return;
    }
    ...
    try {
      ...
      try {
        ...
        log.debug(`Supabase reachable: ${supabaseReachable}`);
        
        if (supabaseReachable) {
          this.retryCount = 0; // Reset retry count on success
        }
      } catch (error) {
        log.debug('Supabase connection test failed:', error);
      ...
      if (supabaseReachable) {
        try {
          log.debug('Testing database availability...');
          databaseAvailable = await this.testWithTimeout(
            testDatabaseAvailability(),
            5000,
            'Database connection timeout'
          );
          log.debug(`Database available: ${databaseAvailable}`);
        } catch (error) {
          log.debug('Database availability test failed:', error);
          // Don't treat missing tables as a connection failure
          if (error instanceof Error && 
              (error.message?.includes('does not exist') || 
               error.message?.includes('relation'))) {
            log.info('Database accessible but schema needs setup');
            databaseAvailable = true; // Allow app to function
          } else {
  ...
  public async retryConnection(): Promise<void> {
    ...
    if (this.retryCount <= this.maxRetries) {
      log.debug(`Retry attempt ${this.retryCount}/${this.maxRetries}, waiting ${delay}ms...`);
