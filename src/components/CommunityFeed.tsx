...
export default function CommunityFeed({ 
  ...
  // Prevent unwanted page refreshes during post creation
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      if (showCreatePost || showShareProject) {
        e.preventDefault();
  ...
  }, [showCreatePost, showShareProject]);
  ...
  // But skip auto-refresh for a short time after creating a post
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (skipNextRefresh) {
        console.log('⏸️ Skipping auto-refresh to prevent interfering with new post');
  ...
  }, [currentUser?.id, refreshPosts, skipNextRefresh]);
  ...
  const handleCreatePost = async (postData: any) => {
    ...
    const newPost: Post = {
      ...
      reflectionNotes: postData.reflectionNotes,
      ...
      author: {
        ...
        profilePicture: currentUser?.profilePicture || 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=150&h=150&fit=crop&crop=face',
    ...
    // Add to the beginning of posts list immediately for instant feedback
    setAllPosts(prev => [newPost, ...prev]);
    
    // Skip the next few auto-refreshes to prevent interference
    setSkipNextRefresh(true);
    
    // Additional safeguard: Set a longer skip period to prevent refresh conflicts
    setTimeout(() => {
      setSkipNextRefresh(false);
      console.log('🔄 Auto-refresh re-enabled after post creation');
    }, 45000); // Skip auto-refresh for 45 seconds after post creation
    
    console.log('✅ Post created successfully and added to feed');
  };

  const handleShareProject = async (shareData: any) => {
    console.log('🚀 Sharing project:', shareData);
    
    // Create a project share post
    const projectPost: Post = {
      ...
      project: {
        ...
        name: shareData.title.split(' - ')[0] || 'Shared Project',
    ...
    // Add to the beginning of posts list immediately for instant feedback
    setAllPosts(prev => [projectPost, ...prev]);
    ...
    setTimeout(() => {
      setSkipNextRefresh(false);
      console.log('🔄 Auto-refresh re-enabled after project sharing');
    }, 45000); // Skip auto-refresh for 45 seconds after project sharing
    
    console.log('✅ Project shared successfully and added to feed');
  };
  ...
  const handleViewPostDetails = (post: Post) => {
    ...
    console.log('👁️ Opening post detail view:', post.id);
  };
      ...
      <div className="space-y-4">
        {posts.map(post => (
          <PostCard
            ...
            onViewDetails={() => handleViewPostDetails(post)}
