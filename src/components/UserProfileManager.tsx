...
const DEMO_PROFILE_DATA: ProfileFormData = {
  ...
  company: 'DevTrack Africa',
  location: 'Lagos',
  country: 'Nigeria',
  bio: 'Passionate developer building the next generation of African tech solutions. I specialize in modern web technologies and love collaborating on impactful projects that make a difference in our communities.',
  ...
  interests: ['Open Source', 'AI/ML', 'Fintech', 'Education Technology'],
  work_experience: [
    {
      id: '1',
      title: 'Full Stack Developer',
      ...
      description: 'Building scalable web applications using React and Node.js. Leading frontend development and mentoring junior developers. Working on projects that impact African developers and businesses.',
      current: true
    },
    {
      id: '2',
...
export default function UserProfileManager({ currentUserId, onBack, onSave, initialProfile }: UserProfileManagerProps) {
  ...
  const handleDeleteProfile = async () => {
    try {
      // Reset to demo data (actual deletion would be handled by the parent component via Supabase)
      setFormData(DEMO_PROFILE_DATA);
      setShowDeleteDialog(false);
      setUnsavedChanges(false);
      
      console.log('Profile reset successfully - deletion would be handled by parent component via Supabase database');
    } catch (error) {
