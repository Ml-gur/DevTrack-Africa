...
interface ProjectCardProps {
  ...
  onClick?: (project: Project) => void; // Alternative to onViewProject for simpler usage
  showActions?: boolean;
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              {project.isCollaborative && safeProject.collaborators.length > 0 && (
                <Badge variant="outline" className="text-blue-600 border-blue-300">
                  <Users className="w-3 h-3 mr-1" />
                  {safeProject.collaborators.length + 1} collaborators
                </Badge>
              )}
          ...
          <div className="mt-4 cursor-pointer" onClick={handleViewProject}>
            {safeProject.images.length === 1 ? (
              ...
              <div className="grid grid-cols-2 gap-2">
                {safeProject.images.slice(0, 4).map((image, index) => (
                  <div key={index} className="relative">
                    ...
                    />
                    {index === 3 && safeProject.images.length > 4 && (
                      <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center rounded-lg">
                        <span className="text-white">
                          +{safeProject.images.length - 4}
                        </span>
                      </div>
                    )}
                  </div>
      ...
      <CardContent className="space-y-4">
        ...
        {project.isCollaborative && safeProject.collaborators.length > 0 && (
          <div className="flex items-center space-x-2">
            ...
            <div className="flex -space-x-2">
              {safeProject.collaborators.slice(0, 3).map((collaborator, index) => (
                <div key={collaborator.id} className="relative">
                  <Avatar className="w-6 h-6 border-2 border-background">
                    <ImageWithFallback
                      src={collaborator.profilePicture || ''}
                      alt={collaborator.fullName}
                      fallback={`https://ui-avatars.com/api/?name=${encodeURIComponent(collaborator.fullName)}&background=f0f0f0&color=333&size=24`}
                    />
                  </Avatar>
                </div>
              ))}
              {safeProject.collaborators.length > 3 && (
                <div className="flex items-center justify-center w-6 h-6 bg-muted border-2 border-background rounded-full">
                  <span className="text-xs">+{safeProject.collaborators.length - 3}</span>
                </div>
              )}
            </div>
          </div>
        ...
        {safeProject.techStack.length > 0 && (
          <div className="flex flex-wrap gap-1">
            {safeProject.techStack.slice(0, 4).map(tech => (
              <Badge key={tech} variant="outline" className="text-xs">
                {tech}
              </Badge>
            ))}
            {safeProject.techStack.length > 4 && (
        ...
        {showComments && (
          <div className="space-y-4 pt-4 border-t">
            {/* Comments List */}
            {safeProject.comments.length > 0 && (
              <div className="space-y-3 max-h-48 overflow-y-auto">
                {safeProject.comments.map(comment => (
